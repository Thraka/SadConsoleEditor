...
namespace SadConsoleEditor
{
    ...
    public class ResizableObject
    {
        ...
        public void ResizeObject(int width, int height, int? positionX = null, int? positionY = null)
        {
            if (objectType == ObjectType.Zone)
            {
                if (Entity.Animation.Width != width || Entity.Animation.Height != height)
                {
                    ...
                    var newAnimation = new AnimatedConsole(Entity.Animation.Name, width, height);
        ...
        {
            if (overlay == null)
                overlay = new Entity(1, 1, SadConsoleEditor.Config.Program.ScreenFont);
            ...
            {
                overlay.Animation = new AnimatedConsole("default", Entity.Animation.Width + 2, Entity.Animation.Height + 2, SadConsoleEditor.Config.Program.ScreenFont);
                overlay.Animation.CreateFrame();
            }

            overlay.Animation.DrawBox(new Rectangle(0, 0, overlay.Animation.Width, overlay.Animation.Height), new Cell(overlay.Animation.DefaultForeground, overlay.Animation.DefaultBackground, 177));

            var centers = new Point(overlay.Animation.Width / 2, overlay.Animation.Height / 2);

            if (Rules.AllowLeftRight && Rules.AllowTopBottom)
            {
                overlay.Animation.SetGlyph(0, 0, 254);
                overlay.Animation.SetGlyph(overlay.Animation.Width - 1, 0, 254);
                overlay.Animation.SetGlyph(overlay.Animation.Width - 1, overlay.Animation.Height - 1, 254);
                overlay.Animation.SetGlyph(0, overlay.Animation.Height - 1, 254);
            }

            if (Rules.AllowLeftRight)
            {
                overlay.Animation.SetGlyph(0, centers.Y, 254);
                overlay.Animation.SetGlyph(overlay.Animation.Width - 1, centers.Y, 254);
            }

            if (Rules.AllowTopBottom)
            {
                overlay.Animation.SetGlyph(centers.X, 0, 254);
                overlay.Animation.SetGlyph(centers.X, overlay.Animation.Height - 1, 254);
            }

            if (objectType == ObjectType.Entity)
            {
                overlay.Animation.SetGlyph(Entity.Animation.Center.X + 1, Entity.Animation.Center.Y + 1, '*');
                overlay.Animation.SetBackground(Entity.Animation.Center.X + 1, Entity.Animation.Center.Y + 1, Color.Black);
