    ...
    class TextTool : ITool
    {
        ...
        public bool ProcessKeyboard(Keyboard info, Console surface)
        {
            if (writing)
            {
                ...
                else
                {
                    ...
                    tempConsole.Cursor.PrintAppearance = new Cell(CharacterPickPanel.SharedInstance.SettingForeground, CharacterPickPanel.SharedInstance.SettingBackground);
        ...
        public void ProcessMouse(MouseConsoleState info, Console surface, bool isInBounds)
        {
            if (info.IsOnConsole && info.Mouse.LeftClicked)
            {
                ...
                if (surface is ScrollingConsole scrolling)
                {
