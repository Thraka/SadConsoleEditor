        ...
        public NoDrawSurface GetCharacter(int glyph, Color alternateForeground, Color alternateBackground)
        {
            ...
            if (newGlyph > 0 && newGlyph < 94)
            {
                if (CharactersSupported[newGlyph])
                {
                    ...
                    var surface = new NoDrawSurface(character.Width, character.Rows.Length);
                    ...
                    foreach (var row in character.Rows)
                    {
                        ...
                        {
                            editor[x, y].Glyph = c.Character;

                            if (Type == FontType.Color)
                            {
                                editor[x, y].Foreground = c.Foreground;
                                editor[x, y].Background = c.Background;
                            }
                            else
                            {
                                editor[x, y].Foreground = alternateForeground;
                                editor[x, y].Background = alternateBackground;
                            }
                            x++;
                        }
